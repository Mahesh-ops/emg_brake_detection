...
int main(void) {
    ...
    uint32_t brake_led_start_time = 0;
    ...
    while (1) {
        ...
        if (brake_intent_flag == 1) {
            ...
            // Delegate to Output Manager: Turns RED ON and GREEN OFF
            OutputMgr_SetBrakes(1);            
            
            brake_led_start_time = GetSystemTime(); // Record the exact time
            is_intent_active = 1;              // Mark the intent as active
        }

        // 4. CHECK LED TIMER (500ms TIMEOUT)
        if (is_intent_active && ((GetSystemTime() - brake_led_start_time) >= 500)) {
            
            // Delegate to Output Manager: Turns RED OFF and GREEN ON
            OutputMgr_SetBrakes(0);            
